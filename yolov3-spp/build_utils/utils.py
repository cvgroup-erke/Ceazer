...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    if GIoU or DIoU or CIoU:
        ...
        if DIoU or CIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            # convex diagonal squared 对角线距离平方
            c2 = cw ** 2 + ch ** 2 + 1e-16
            # centerpoint distance squared 中心点距离平方
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            ciou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, CIoU=True)  # giou(prediction, target)
            lbox += (1.0 - ciou).mean()  # giou loss

            # Obj
            # tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * ciou.detach().clamp(0).type(tobj.dtype)  # ciou ratio
            tobj[b, a, gj, gi] = 1.0
        ...
        lobj += BCEobj(pi[..., 4], tobj)  # obj loss   confidence
...
def build_targets(p, targets, model):
    ...
    nt = targets.shape[0]   # 一个batch图片的目标个数
