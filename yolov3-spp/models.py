...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p):
        ...
        # p返回网络预测值，tx,ty,tw,th，还需decode的过程
        ...
        if self.training:
            ...
            torch.sigmoid_(io[..., 4:])
            return io.view(bs, -1, self.no), p  # view [bs, 3, 13, 13, 85] as [1, 507, 85]
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        # 解析网络对应的.cfg文件
        self.module_defs = parse_model_cfg(cfg)  # 返回一个列表，列表里面都是字典，，每一个字典就是一层的配置
        # 根据解析的网络结构一层一层去搭建
