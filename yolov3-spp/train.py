...
def train(hyp):
    ...
    grid_min, grid_max = imgsz_test // gs, imgsz_test // gs      # 16  16
    if multi_scale:
        imgsz_min = opt.img_size // 1.5         # 341
        imgsz_max = opt.img_size // 0.667       # 767

        # 将给定的最大，最小输入尺寸向下调整到32的整数倍
        grid_min, grid_max = imgsz_min // gs, imgsz_max // gs      # 10   23
        imgsz_min, imgsz_max = int(grid_min * gs), int(grid_max * gs)   # 320   736
    ...
    val_datasetloader = DataLoader(val_dataset,
                                   batch_size=batch_size,
                                   num_workers=nw,
                                   pin_memory=True,
        ...
        if opt.notest is False or epoch == epochs - 1:
            ...
            if opt.savebest is False:
                ...
                with open(results_file, 'r') as f:
                    ...
                    torch.save(save_files, "./weights/yolov3spp-{}-{:.3f}.pt".format(epoch, best_map))
            else:
                ...
                if best_map == coco_mAP:
                    with open(results_file, 'r') as f:
                        ...
                        if opt.amp:
                            save_files["scaler"] = scaler.state_dict()
                        torch.save(save_files, best.format(best_map))
